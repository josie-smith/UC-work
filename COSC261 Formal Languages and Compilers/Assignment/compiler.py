...
'''
To test, you need a program and some test input.
py compiler.py < program > program.j
java -Xmx100m -jar jasmin.jar program.j
java -Xmx100m Program < test_input
'''
...
class Scanner:
    ...
    def skip_white_space(self):
        ...
        match = re.match('\s', self.input_string[self.current_char_index:])
        while match:
            self.current_char_index += len(match.group())
            match = re.match('\s', self.input_string[self.current_char_index:])
    ...
    def get_token(self):
        ...
        if len(self.input_string[self.current_char_index:]) > 0 and token == None:
            self.no_token()
    ...
    def consume(self, *expected_tokens):
        ...
        if next_token in expected_tokens:
            if next_token in [Token.NUM, Token.ID]:
                self.current_token = self.get_token()
            ...
            self.unexpected_token(next_token, expected_tokens)
...
class If_Else_AST:
   ...
   def __repr__(self):
       return 'if ' + repr(self.condition) + ' then ' + \
                      repr(self.then) + ' else ' + repr(self.Else) + ' end'
   def indented(self, level):
...
class BooleanExpression_AST:
    ...
    def __repr__(self):
        ...
        for ex in self.expressions[1:]:
            result += Token.OR + repr(ex)
        return result
    def indented(self, level):
        if len(self.expressions) == 1:
            result = self.expressions[0].indented(level)
        else:
            result = indent(Token.OR, level)
            for ex in self.expressions:
                result += self.expressions[0].indented(level+1)
        return result
    def true_code(self, label):
        expressions = ""
        for ex in self.expressions:
            expressions += ex.true_code(label)
        return comps
    def false_code(self, label):
        l1 = label_generator.next()
        expressions = ""
        for ex in self.expressions:
            expressions += ex.true_code(l1)
        return expressions + 'goto ' + label + '\n' + l1 + ':\n'
...
class BooleanTerm_AST:
    ...
    def __repr__(self):
        ...
        for term in self.terms[1:]:
            result += Token.AND + repr(term)
        return result
    def indented(self, level):
        if len(self.terms) == 1:
            result = self.terms[0].indented(level)
        else:
            result = indent(Token.AND, level)
            for ex in self.terms:
                result += self.terms[0].indented(level+1)
        return result
    def true_code(self, label):
        l1 = label_generator.next()
        comparisons = ''
        for term in self.terms:
            comparisons += term.false_code(l1)
        return comparisons + 'goto ' + label + '\n' + l1 + ':\n'
    def false_code(self, label):
        comps = ''
...
class  BooleanFactor_AST:
    def __init__(self, factor):
        self.factor = factor
    def __repr__(self):
        return Token.NOT + repr(self.factor)
    def indented(self, level):
        return indent(Token.NOT, level) + indent(self.factor, level+1)
    def true_code(self, label):
...
def boolean_expression():
    expressions = [boolean_term()]
    while scanner.lookahead() == Token.OR:
        scanner.consume(Token.OR)
        expressions.append(boolean_term())
    return BooleanExpression_AST(expressions)
